// tiny_llm_mxint8_fi.c — TinyLLM for FI campaigns (MXLOG8_LOGDOM)
...
#include "print.h"
#include "mx.h"
#include "weights/weights_mxlog8_logdom_llm.h"
...
static inline uint32_t matvec4_int8(const uint32_t *w, uint32_t x, int shift) {
    ...
    MXLOGDOM_DOT4(d0, x, w[0]);
    MXLOGDOM_DOT4(d1, x, w[1]);
    MXLOGDOM_DOT4(d2, x, w[2]);
    MXLOGDOM_DOT4(d3, x, w[3]);
    return MX_PACK4(
...
int main() {
    uart_init();
    printf("TINY_LLM_MXLOG8_LOGDOM_FI\n");
    uart_write_flush();
    ...
    const uint32_t *W = llm_w_mxlog8_logdom;
    ...
    for (int pos = 0; pos < LLM_PROMPT_LEN + LLM_GEN_LEN; pos++) {
        ...
        for (int l = 0; l < LLM_N_LAYERS; l++) {
            ...
            MXLOGDOM_ADD4(x_post_attn, x_residual, attn_out);
            uint32_t x_ffn_residual = x_post_attn;

            int32_t hidden[LLM_D_FFN];
            for (int h = 0; h < LLM_D_FFN; h++) {
                uint32_t dot;
                MXLOGDOM_DOT4(dot, x_post_attn, W[OFF_WUP(l) + h]);
            ...
            for (int d = 0; d < LLM_D_MODEL; d++) {
                ...
                for (int g = 0; g < LLM_D_FFN / 4; g++) {
                    ...
                    uint32_t dot;
                    MXLOGDOM_DOT4(dot, h_packed, W[OFF_WDN(l) + d*4 + g]);
            ...
            MXLOGDOM_ADD4(x, x_ffn_residual, ffn_packed);
        }
        ...
        if (pos >= LLM_PROMPT_LEN - 1 && pos < LLM_PROMPT_LEN + LLM_GEN_LEN - 1) {
            ...
            for (int t = 0; t < LLM_VOCAB_SIZE; t++) {
                uint32_t dot;
                MXLOGDOM_DOT4(dot, x, W[OFF_LM_HEAD + t]);

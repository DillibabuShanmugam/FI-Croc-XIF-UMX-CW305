#!/usr/bin/env python3
"""Clock glitch on QNN workload — all 5 MX formats via CW-Lite.

Uses cw305_live bitstreams (QNN firmware baked in).
J16=1 (tio_clkin from CW-Lite), CW-Lite connected.

Protocol: reprogram-per-trial, width sweep + dense boundary sweep.
"""
...
def run_glitch_trial(scope, bitstream, width, offset, ext_offset):
...
for fmt in FORMATS:
    ...
    golden = GOLDEN[fmt]
    print(f'\n{"="*60}')
    print(f'  Clock Glitch: {fmt} (golden={golden})')
    print(f'{"="*60}')

    rows = []

    # Phase 1: Boundary find (width sweep, 3 trials each)
    print('  Phase 1: Width sweep w=-5 to -24, ext_offset=50')
    for w in range(-5, -25, -1):
        for trial in range(3):
            d = run_glitch_trial(scope, bitstream, w, 0, 50)
            cls = classify(d['eoc'], d['ret'], golden)
            row = {'workload': 'qnn', 'format': fmt,
        ...
        if all(c == 'dead' for c in classes):
            print(f'    Dead at w={w}, stopping width sweep')
            break

    # Phase 2: Dense ext_offset sweep at interesting widths
    print('  Phase 2: Dense sweep w=-8,-9,-10, ext=50-500 step 50')
    for w in [-8, -9, -10]:
        for ext in range(50, 550, 50):
            d = run_glitch_trial(scope, bitstream, w, 0, ext)
            cls = classify(d['eoc'], d['ret'], golden)
            row = {'workload': 'qnn', 'format': fmt,
    ...
    csvpath = f'{OUTDIR}/clkglitch_{fmt}_qnn.csv'
